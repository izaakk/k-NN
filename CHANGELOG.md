...
* Add Intel SVS Vamana method (`svs_vamana`) as the first experimental `:sandbox` tenant, with `flat`/`sq`/`lvq` encoders, `cosinesimil`, and `alpha` (opt-in via `FAISS_ENABLE_SVS`) [#XXXX](https://github.com/opensearch-project/k-NN/pull/XXXX)
...
* Bump Faiss pin to 67f066f7a (SVS v0.3.0 + post-release fixes); regenerate 10 Faiss patches against the new pin [#XXXX](https://github.com/opensearch-project/k-NN/pull/XXXX)
