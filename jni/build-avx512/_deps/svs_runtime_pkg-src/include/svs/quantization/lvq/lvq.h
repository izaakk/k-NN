...
class LVQDataset {
  public:
    ...
    static constexpr bool is_blocked = detail::is_blocked<Alloc>;
    using element_type = ScaledBiasedDataset<Primary, Extent, Strategy, Alloc>;
    using strategy = Strategy;
    ...
    using allocator_type = Alloc;
    using lib_alloc_data_type =
        LVQDataset<Primary, Residual, Extent, Strategy, lib::Allocator<std::byte>>;
    using lib_blocked_alloc_data_type = LVQDataset<
        Primary,
        Residual,
        Extent,
        Strategy,
        data::Blocked<lib::Allocator<std::byte>>>;
  ...
  public:
    ...
    using const_residual_value_type = typename residual_type::const_value_type;
    ...
    /// @brief Access only the residual dataset.
    const_residual_value_type get_residual(size_t i) const;

    /// @brief Access the entire primary dataset.
    ...
    void set_primary(size_t i, const const_primary_value_type& v);

    void set_residual(size_t i, const const_residual_value_type& v);
    ...
    static LVQDataset load(const lib::LoadTable& table, const allocator_type& allocator);
};
...
class LVQDataset<Primary, 0, Extent, Strategy, Alloc> {
  public:
    ...
    using centroid_type = detail::centroid_type;
    using lib_alloc_data_type =
        LVQDataset<Primary, 0, Extent, Strategy, lib::Allocator<std::byte>>;
    using lib_blocked_alloc_data_type =
        LVQDataset<Primary, 0, Extent, Strategy, data::Blocked<lib::Allocator<std::byte>>>;
...
template <typename T>
concept IsLVQDataset = is_lvq_dataset<std::remove_cvref_t<T>>;

template <typename T>
concept IsTwoLevelDataset = is_lvq_dataset<std::remove_cvref_t<T>> &&
                            (std::remove_cvref_t<T>::residual_bits != 0);
