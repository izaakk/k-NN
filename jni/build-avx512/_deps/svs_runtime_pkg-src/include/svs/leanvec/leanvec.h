...
template <size_t Extent> struct LeanVecMatrices {
  public:
    ...
    using leanvec_matrix_view_type = data::ConstSimpleDataView<float, Extent>;
    ...
    leanvec_matrix_view_type view_data_matrix() const;
    leanvec_matrix_view_type view_query_matrix() const;
...
class LeanDataset {
  public:
    ...
    using element_type = primary_data_type;
    using lib_alloc_data_type =
        LeanDataset<T1, T2, LeanVecDims, Extent, lib::Allocator<std::byte>>;
    using lib_blocked_alloc_data_type =
        LeanDataset<T1, T2, LeanVecDims, Extent, data::Blocked<lib::Allocator<std::byte>>>;
    ...
    /// @brief Default method accesses primary/LeanVec dataset.
    const_value_type get_primary(size_t i) const;
    ...
    leanvec_matrices_type get_leanvec_matrices() const;
    ...
    /// @brief Only set the primary dataset.
    void set_primary(size_t i, const const_value_type& v);

    /// @brief Only set the secondary dataset.
    void set_secondary(size_t i, const secondary_const_value_type& v);
    ...
    static LeanDataset load(const lib::LoadTable& table, const allocator_type& allocator);
...
template <
    ...
    threads::ThreadPool Pool>
LeanVecMatrices<Extent> compute_leanvec_matrices_ood(
    const Dataset& data,
    const Queries& queries,
    Pool& threadpool,
    lib::MaybeStatic<LeanVecDims> leanvec_dims,
    size_t num_max_steps = 500,
    float rel_tol = 1e-3
);
