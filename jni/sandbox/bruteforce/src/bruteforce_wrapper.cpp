...
jlong knn_jni::bruteforce_wrapper::LoadIndexWithStream(knn_jni::JNIUtilInterface *jniUtil, JNIEnv *env, jobject readStreamJ) {
    ...
    // Same discipline as readString's length cap: never size an allocation from an unvalidated header.
    // The caps are generous for an example engine; a corrupt or truncated file must fail here, not as a
    // multi-gigabyte allocation.
    if (index->dim <= 0 || index->dim > 16000 || count < 0 || count > 100000000) {
        throw std::runtime_error("bruteforce: corrupt header in serialized index");
    }
...
jobjectArray knn_jni::bruteforce_wrapper::QueryIndex(knn_jni::JNIUtilInterface *jniUtil, JNIEnv *env, jlong indexPointer,
    ...
    // Copy the query into a local buffer instead of pinning: a throw below (allocation failure on a
    // large scan) then cannot leak a pinned array, and there is no release call to forget.
    std::vector<jfloat> query(index->dim);
    env->GetFloatArrayRegion(queryVectorJ, 0, index->dim, query.data());
    jniUtil->HasExceptionInStack(env, "bruteforce: reading the query vector failed");

    // Exact scan by design: the demo materializes one (distance, id) pair per scanned row, O(scanRows)
    // memory per query. A production tenant would keep a k-bounded heap instead.
    ...
    for (size_t row = 0; row < scanRows; row++) {
        ...
        closest.emplace_back(squaredL2Distance(query.data(), rowVector, index->dim), index->ids[row]);
    }
