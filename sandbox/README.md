...
exists for), and doing it well needs host-side knowledge: the faiss build must be static + PIC with a
SIMD-variant choice mirroring the core's `init-faiss.cmake` detection, and BLAS/LAPACK/OpenMP re-supplied
on the link line (a static faiss archive doesn't carry them). The shared `knn_sandbox_vendor_faiss` helper
packages that exact recipe, so a faiss tenant's vendoring is a one-liner instead of the generic
`ExternalProject_Add` above:

```cmake
# Vendor YOUR OWN faiss at YOUR OWN pinned commit — not the k-NN submodule, no k-NN patches. This is how a
# tenant exposes index types that exist in faiss mainstream but not yet in the k-NN pin. Any extra flags
# (e.g. -DFAISS_ENABLE_SVS=ON) go in CMAKE_ARGS.
knn_sandbox_vendor_faiss(acme_faiss
    GIT_TAG 0123456789abcdef0123456789abcdef01234567
)
...
knn_sandbox_add_jni_library(opensearchknn_acme
    ...
    INCLUDE_DIRS
        ${KNN_SANDBOX_TENANT_DIR}/include
        ${acme_faiss_INCLUDE_DIR}
    LINK_LIBRARIES
        ${acme_faiss_LINK_LIB}
    DEPENDS
        ${acme_faiss_EP_TARGET}
)
```

It exports `<prefix>_INCLUDE_DIR`, `<prefix>_LINK_LIB` (an imported static lib carrying BLAS/LAPACK/OpenMP
on its interface) and `<prefix>_EP_TARGET`; the `faiss_upstream` tenant's `tenant.cmake` is the in-tree
worked example.
...
* ⚠ Only your tenant **jar** (plus `:sandbox:common`'s) is bundled into the plugin zip — an
  `implementation` dependency on anything OpenSearch does not already provide will be missing at runtime.
  Keep tenant runtime dependencies to OpenSearch-provided libraries, or vendor the code.
