...
```java
public class AcmeNativeEngineService implements NativeEngineService {
    ...
    // ... implement insertToIndex/writeIndex/loadIndex/queryIndex/free the same way ...
    // For operations your engine does NOT support (template builds, radial search, ...) throw a
    // descriptive UnsupportedOperationException, exactly as the fixture does — declining is your
    // engine's decision, not the core's. The corresponding capability flags (step 2) normally keep
    // those paths unreachable; the throws are the defensive backstop behind them. (A pure-JVM
    // tenant can skip the native service entirely — KNNEngineDefinition.nativeService() defaults
    // to null.)
}
```
...
public class AcmeService {
    ...
    static {
        ...
        // (loadLibraryByVariant is a ~10-line generalization of KNNLibraryLoader.loadFaissLibrary that
        // ships alongside the first native tenant — your tenant PR adds it if it is not there yet.)
