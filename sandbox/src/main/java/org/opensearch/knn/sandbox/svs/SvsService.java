 ...
 * this class.
 *
 * <p>The native surface is deliberately minimal: exactly the operations an {@code svs_vamana} index needs
 * (build iteratively, write, load, top-k query with an optional pre-filter, free). There are no binary/byte,
 * training, template, shared-state or radial entry points — those engine features are not part of the SVS
 * scope and are rejected at the Java layer.
 */
@Log4j2
