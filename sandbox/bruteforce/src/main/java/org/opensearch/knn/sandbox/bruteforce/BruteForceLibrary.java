...
/**
 * The example engine's {@link org.opensearch.knn.index.engine.KNNLibrary}: one method ({@code scan}, an
 * exact scan), L2 float vectors, x1 compression, iterative build, custom segment files. Everything not
 * declared here keeps the conservative {@link NativeLibrary} defaults.
 */
public final class BruteForceLibrary extends NativeLibrary {
    ...
    /** Key the indexing context below uses to hand the space type to the engine service. */
    ...
    /** The single method. L2 + float only; no build parameters, no query parameters, no training. */
    private static final class ScanMethod implements KNNMethod {
        ...
        public KNNLibraryIndexingContext getKNNLibraryIndexingContext(
            ...
            // The field mapper consumes the validators/processors from this context; a tenant must supply
            // them (these are the core defaults) or vector parsing fails. Candidate SPI refinement.
        ...
        public KNNLibrarySearchContext getKNNLibrarySearchContext() {
            return ctx -> Map.of(); // no engine-specific query parameters
        }
    }
}
