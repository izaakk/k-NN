...
public class FixtureJNIServiceDispatchTests extends OpenSearchTestCase {
    ...
    public void testIndexLifecycleDispatchesToFixtureService() throws Exception {
        ...
        // The bytes written through the pure-Java write path came back intact through the read path.
        assertTrue(opLog.get(4).startsWith("loadIndex(bytes=" + FixtureNativeEngineService.FIXTURE_SEGMENT_BYTES.length + ")"));
    ...
    public void testOnHeapVectorInsertDispatchesToFixtureService() {
        // A pure-JVM engine declares prefersJavaVectors() and receives its vector batches on-heap: the
        // float[][] insertToIndex overload must route to the engine's service with the batch intact.
        assertTrue(fixtureEngine.getNativeService().prefersJavaVectors());
        final long initHandle = JNIService.initIndex(2, 2, Map.of(), fixtureEngine);
        JNIService.insertToIndex(new int[] { 0, 1 }, new float[][] { { 1f, 2f }, { 3f, 4f } }, Map.of(), initHandle, fixtureEngine);

        final List<String> opLog = fixtureService.opLog();
        assertEquals(2, opLog.size());
        assertTrue(opLog.get(1).startsWith("insertToIndex(docs=2, vectors=2, dim=2"));
    }

    public void testOnHeapVectorInsertRejectsEnginesWithoutNativeService() {
        // Built-ins never take the on-heap path: no NativeEngineService means the overload has no target.
        expectThrows(
            IllegalArgumentException.class,
            () -> JNIService.insertToIndex(new int[] { 0 }, new float[][] { { 1f, 2f } }, Map.of(), 42L, KNNEngine.FAISS)
        );
