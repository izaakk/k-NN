...
public class JNIService {
    ...
    public static long initIndex(long numDocs, int dim, Map<String, Object> parameters, KNNEngine knnEngine) {
        if (knnEngine.getNativeService() != null) {
            return knnEngine.getNativeService().initIndex(new NativeIndexBuildParams(numDocs, dim, false, EngineParameters.of(parameters)));
        }
        ...
        if (knnEngine.getNativeService() != null) {
            knnEngine.getNativeService()
                .insertToIndex(
                    ...
                    vectorsAddress,
                    indexAddress,
                    new NativeIndexBuildParams(0, dimension, false, EngineParameters.of(parameters))
                );
            return;
        }
    ...
    public static void writeIndex(
        ...
        if (knnEngine.getNativeService() != null) {
            knnEngine.getNativeService()
                .writeIndex(output, indexAddress, new NativeIndexBuildParams(0, 0, skipFlat, EngineParameters.of(parameters)));
            return;
        }
        if (KNNEngine.FAISS == knnEngine) {
            if (IndexUtil.isBinaryIndex(knnEngine, parameters)) {
        ...
        if (knnEngine.getNativeService() != null) {
            knnEngine.getNativeService()
                .createIndexFromTemplate(
                    ...
                    output,
                    templateIndex,
                    new NativeIndexBuildParams(0, dim, false, EngineParameters.of(parameters))
                );
            return;
        }
        if (KNNEngine.FAISS == knnEngine) {
    ...
    public static long loadIndex(IndexInputWithBuffer readStream, Map<String, Object> parameters, KNNEngine knnEngine) {
        if (knnEngine.getNativeService() != null) {
            return knnEngine.getNativeService()
                .loadIndex(readStream, new NativeIndexBuildParams(0, 0, false, EngineParameters.of(parameters)));
        }
        if (KNNEngine.FAISS == knnEngine) {
            if (IndexUtil.isBinaryIndex(knnEngine, parameters)) {
