 ...
 * excluded from default-build integTest runs. LeanVec is deferred to the training follow-up. SVS names are
 * written as literals to avoid a test-time dependency on the sandbox module.
 */
public class FaissSVSVamanaIT extends KNNRestTestCase {
    ...
    // Radial search is not supported by the SVS engine (pending an SVS runtime fix); a min_score query must
    // be rejected with a validation error, not reach the native layer.
    @SneakyThrows
